...
function renderRoleWorkspace() {
    ...
    const role = String(FACULTY_DATA.normalized_role || FACULTY_DATA.role || "STAFF").toUpperCase();
...
async function loadFacultyNotifications() {
    ...
    try {
        ...
        el.innerHTML = list.map((n) => {
            ...
            return `
                <li class="fd-note ${unread ? "unread" : ""}">
                    <div class="fd-note-head">
                        <div class="fd-note-title">
                            <span class="fd-note-title-text">${title}</span>
                            ${unread ? '<span class="fd-note-badge">Unread</span>' : ""}
                        </div>
                        <time class="fd-note-time" datetime="${created}">${created}</time>
                    </div>
                    ${message ? `<div class="fd-note-msg">${message}</div>` : ""}
